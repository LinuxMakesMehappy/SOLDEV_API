...
A serverless backend service that transforms Solana and Anchor error codes into human-readable explanations with actionable fix suggestions. Built with multiple fallback layers to ensure high availability.

## Features

- **AI-Enhanced Explanations**: Uses AWS Bedrock (Claude-3) and external AI APIs for contextual error analysis
- **High Performance**: DynamoDB-based caching for sub-500ms response times
